create proc sp_VerificateDocData
	@TaskID varchar(50),
	@BeginDate DATETIME,
	@EndDate datetime,
	...
	@DetailLevel int=0,							--明细级别，0为公司级别，1为部门级别，2为仓库级别，3为单据级别，99为最末级别
	@RecordLog bit=1,						--
	@Remark varchar(500)='',
	@Usercode varchar(30)='',
...
as
	BEGIN
		...
		*/
		...
		1.检查单据总额与明细金额是否一致
		*/
		if @FormIDList='' or exists(select 1 from commondb.dbo.split(@FormIDList,',') s where s.List in(2419,2420))
			BEGIN
				select 
			END
